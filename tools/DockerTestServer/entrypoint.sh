...
# =================
# What this will do:
# 1) Copy the default config/ files from the read-only volume to /shiptest/config/
# 2) Override files under /shiptest/config/ with files present in /gamecfg/
# 3) Process environment variables passed to the container into /shiptest/config/
...
sed -i -r 's/(#|^)ADDRESS .*/ADDRESS '"$DB_HOST"'/' /shiptest/config/dbconfig.txt
sed -i -r 's/(#|^)FEEDBACK_LOGIN .*/FEEDBACK_LOGIN '"$DB_USER"'/' /shiptest/config/dbconfig.txt
sed -i -r 's/(#|^)FEEDBACK_PASSWORD .*/FEEDBACK_PASSWORD '"$DB_PASS"'/' /shiptest/config/dbconfig.txt
...
envvar_override "TG_GAME_" "/shiptest/config/game_options.txt"
...
cd /shiptest
DreamDaemon shiptest.dmb -port 1337 -trusted -close -verbose
