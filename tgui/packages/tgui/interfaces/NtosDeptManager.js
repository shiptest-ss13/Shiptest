...
export const NtosDeptManager = (props, context) => {
  return (
    <NtosWindow width={520} height={620} resizable>
    ...
    >
        ...
        <Table>
          ...
          {accounts.map((account) => (
            <Table.Row key={account.id} className="candystripe">
              ...
              <Table.Cell collapsing>
                <AnimatedNumber value={account.balance + 'cr'} />
              </Table.Cell>
              <Table.Cell collapsing>
                <Button
                  color={'bad'}
      ...
      {tab === 2 && (
        <Table>
          ...
          {budgets.map((budget) => (
            <Table.Row key={budget.name} className="candystripe">
