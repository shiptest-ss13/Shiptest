...
/mob/living/attacked_by(obj/item/attacking_item, mob/living/user, modifier = 1)
	...
	if(!attacking_item.force)
		return FALSE
	apply_damage(attacking_item.force * modifier, attacking_item.damtype, break_modifier = attacking_item.force * modifier, blocked = armor_value, sharpness = attacking_item.get_sharpness()) //Bone break modifier = item force
