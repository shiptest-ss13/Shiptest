	...
	for(var/v in stored_research.researched_designs)
		...
		if(D.build_type & MECHFAB)
			...
			if(check_resources(D))
				output += "<a href='byond://?src=[REF(src)];part=[D.id]'>Build</a> | "
			output += "<a href='byond://?src=[REF(src)];add_to_queue=[D.id]'>Add to queue</a><a href='byond://?src=[REF(src)];part_desc=[D.id]'>?</a></div>"
	return output
...
/obj/machinery/mecha_part_fabricator/proc/output_ui_header()
	...
	if (rmat.mat_container)
		output += "<a href='byond://?src=[REF(src)];screen=resources'><B>Material Amount:</B> [rmat.format_amount()]</A>"
	else
		output += "<font color='red'>No material storage connected, please contact the quartermaster.</font>"
	output += "<a href='byond://?src=[REF(src)];screen=main'>Main Screen</a>"
	output += "</div>"
...
/obj/machinery/mecha_part_fabricator/proc/output_ui_materials()
	...
	for(var/mat_id in rmat.mat_container.materials)
		...
		if(amount >= MINERAL_MATERIAL_AMOUNT) output += "<A href='byond://?src=[REF(src)];remove_mat=1;material=[ref]'>Eject</A>"
		if(amount >= MINERAL_MATERIAL_AMOUNT*5) output += "<A href='byond://?src=[REF(src)];remove_mat=5;material=[ref]'>5x</A>"
		if(amount >= MINERAL_MATERIAL_AMOUNT) output += "<A href='byond://?src=[REF(src)];remove_mat=50;material=[ref]'>All</A>"
		output += "<br>"
	output += "</div>"
	return output
...
/obj/machinery/mecha_part_fabricator/proc/list_queue()
	...
	if(!istype(queue) || !queue.len)
		...
		for(var/datum/design/D in queue)
			...
			output += "[i>1?"<a href='byond://?src=[REF(src)];queue_move=-1;index=[i]' class='arrow'>&uarr;</a>":null] "
			output += "[i<queue.len?"<a href='byond://?src=[REF(src)];queue_move=+1;index=[i]' class='arrow'>&darr;</a>":null] "
			output += "<a href='byond://?src=[REF(src)];remove_from_queue=[i]'>Remove</a></li>"
		...
		switch(screen)
			if("main")
				for(var/part_set in part_sets)
					left_part += "<a href='byond://?src=[REF(src)];part_set=[part_set]'>[part_set]</a> - <a href='byond://?src=[REF(src)];partset_to_queue=[part_set]'>Add all parts to queue</a><br>"
			if("parts")
