...
#define DISTRESS_LENGTH 10 SECONDS
#define DISTRESS_COOLDOWN 3 MINUTES // May need to be raised in the future, depending on behavior
		...
		switch(choice)
			...
			if("Broadcast new location name")
				if(!inserted_id || !inserted_id.officer)
					...
					return
				play_click_sound()
				var/new_name = stripped_input(user, "Assign [src] a new name. Enter the new name and ensure that it is no bigger than [MAX_NAME_LENGTH] characters long.", "Location Naming", max_length = MAX_NAME_LENGTH)
				if(!new_name)
					return
				var/datum/overmap/dynamic/OE = get_overmap_location()
				var/old_name = OE.name
				if(!OE.Rename(new_name))
					return
				play_click_sound()
				to_chat(user, "<span class='notice'>You set [old_name]'s name to [OE.name].</span>")

			if("Send distress beacon")
			// Don't forget to check the cooldown
				if(!inserted_id)
...
/obj/machinery/power/planet_beacon/proc/change_state(new_state)
	...
	switch (new_state)
		...
		if(BEACON_DISTRESS) // Send out a distress and temporarily change the screen
			...
			addtimer(CALLBACK(src, PROC_REF(change_state), state), DISTRESS_LENGTH) // Return to the previous state after a certain time
