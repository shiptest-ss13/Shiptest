...
/obj/item/aiModule/proc/install(datum/ai_laws/law_datum, mob/user)
	...
	if(law_datum.owner)
		deadchat_broadcast("<b> changed <span class='name'>[ainame]</span>'s laws at [get_area_name(user, TRUE)].</b>", span_name("[user]") , follow_target=user, message_type=DEADCHAT_LAWCHANGE)
