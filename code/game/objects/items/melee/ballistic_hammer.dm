...
/obj/item/melee/sledgehammer/gorlex/blasting/attack(mob/living/target, mob/living/user)
	. = ..()
	if(check_chambered(TRUE)) // HEY YOSHIHIDE
		target.Knockdown(1)
...
/obj/item/melee/sledgehammer/gorlex/blasting/attack_obj(obj/O, mob/living/user)
	. = ..()
	if(check_chambered(TRUE))
		new /obj/effect/temp_visual/kinetic_blast(target.loc)
		if(istype(O, /obj/machinery/door/airlock))
			var/obj/machinery/door/airlock/breaching = O
			user.visible_message(span_warning("You smash open \the [breaching] with a thundering boom!"), span_warning("[user] smashes open \the [breaching] with \the [src] like it was made of glass!"), span_warning("You hear a defeaning boom."))
			breaching.Destroy()
...
/obj/item/melee/sledgehammer/gorlex/blasting/proc/check_chambered(spend_round = FALSE)
