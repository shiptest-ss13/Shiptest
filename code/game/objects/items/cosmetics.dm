		...
		if(location == BODY_ZONE_PRECISE_MOUTH)
			if(user.a_intent == INTENT_HELP)
				...
				if(H == user) //shaving yourself
					user.visible_message("<span class='notice'>[user] starts to shave [user.p_their()] facial hair with [src].</span>", "<span class='notice'>You take a moment to shave your facial hair with [src]...</span>")
					if(do_after(user, 50, target = H))
						user.visible_message("<span class='notice'>[user] shaves [user.p_their()] facial hair clean with [src].</span>", "<span class='notice'>You finish shaving with [src]. Fast and clean!</span>")
						shave(H, location)
				else
					user.visible_message("<span class='warning'>[user] tries to shave [H]'s facial hair with [src].</span>", "<span class='notice'>You start shaving [H]'s facial hair...</span>")
					if(do_after(user, 50, target = H))
						user.visible_message("<span class='warning'>[user] shaves off [H]'s facial hair with [src].</span>", "<span class='notice'>You shave [H]'s facial hair clean off.</span>")
						shave(H, location)
			...
			if("moth_fluff" in H.dna.species.mutant_bodyparts)
				if(user.a_intent == INTENT_HELP)
					...
					if(H.dna.features["moth_fluff"] == "Shaved")
						...
						return
					if(H == user) //shaving yourself
						user.visible_message("<span class='notice'>[user] starts to shave [user.p_their()] fluff off with [src].</span>", "<span class='notice'>You start to shave your fluff off with [src]...</span>")
						if(do_after(user, 5, target = H))
							user.visible_message("<span class='notice'>[user] shaves [user.p_their()] fluff off with [src].</span>", "<span class='notice'>You finish shaving your fluff off with [src].</span>")
							H.dna.features["moth_fluff"] = "Shaved"
							H.update_body()
							playsound(loc, 'sound/items/welder2.ogg', 20, TRUE)
					else
						var/turf/H_loc = H.loc
						user.visible_message("<span class='warning'>[user] tries to shave [H]'s fluff with [src]!</span>", "<span class='notice'>You start shaving [H]'s fluff...</span>")
						if(do_after(user, 50, target = H))
							if(H_loc == H.loc)
								user.visible_message("<span class='warning'>[user] shaves [H]'s fluff off with [src]!</span>", "<span class='notice'>You shave [H]'s fluff off.</span>")
								H.dna.features["moth_fluff"] = "Shaved"
								H.update_body()
								playsound(loc, 'sound/items/welder2.ogg', 20, TRUE)
