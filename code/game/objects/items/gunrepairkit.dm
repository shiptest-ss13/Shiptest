/obj/item/gun_fixer
	...
	desc = "A toolkit containing everything needed to scrub the frontier-gunk out of a gun and return it to a mostly-usable state. Several chemicals required for a full cleaning process result in each kit being single-use."
...
/obj/item/gun_fixer/proc/accidents_happen(obj/item/gun/ballistic/whoops, mob/darwin)
	...
	if(whoops.internal_magazine && !whoops.magazine.ammo_count(TRUE))
		return
	if(prob(1)) //this gets called I think once per decisecond so we don't really want a high chance here. its like firing the force of nature and expecting it to not explode
		if(whoops.safety)
			whoops.safety = FALSE
