	...
	if(isturf(loc))
		...
		if(amount_grown >= 100)
			...
			if(player_spiders)
				...
				notify_ghosts("Spider [S.name] can be controlled", null, enter_link="<a href='byond://?src=[REF(S)];activate=1'>(Click to play)</a>", source=S, action=NOTIFY_ATTACK, ignore_key = POLL_IGNORE_SPIDER)
			qdel(src)
