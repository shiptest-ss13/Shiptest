...
/datum/component/pellet_cloud/Initialize(projectile_type=/obj/item/shrapnel, magnitude=5)
	if(!isammocasing(parent) && !isgrenade(parent) && !islandmine(parent) && !issupplypod(parent) && !isanimal(parent))
...
/datum/component/pellet_cloud/proc/grenade_moved()
	...
	for(var/mob/living/new_mob in get_turf(parent))
		RegisterSignal(new_mob, COMSIG_QDELETING, PROC_REF(on_target_qdel), override=TRUE)
