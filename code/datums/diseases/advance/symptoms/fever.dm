...
/datum/symptom/fever/proc/set_body_temp(mob/living/M, datum/disease/advance/A)
	if(!unsafe)
		M.add_body_temperature_change("fever", max((6 * power) * A.stage, (HUMAN_BODYTEMP_HEAT_DAMAGE_LIMIT - 1)))
	else
		M.add_body_temperature_change("fever", max((6 * power) * A.stage, (HUMAN_BODYTEMP_HEAT_DAMAGE_LIMIT + 20)))
