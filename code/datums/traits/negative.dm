...
/datum/quirk/badback/on_process(delta_time)
...
/datum/quirk/family_heirloom/on_process(delta_time)
...
/datum/quirk/nyctophobia/on_process(delta_time)
	var/mob/living/carbon/human/H = quirk_holder
...
/datum/quirk/insanity/on_process(delta_time)
	if(quirk_holder.reagents.has_reagent(/datum/reagent/toxin/mindbreaker, needs_metabolizing = TRUE))
		...
		return
	if(SPT_PROB(2, delta_time)) //we'll all be mad soon enough
		madness()
...
/datum/quirk/junkie/on_process(delta_time)
	var/mob/living/carbon/human/H = quirk_holder
...
/datum/quirk/junkie/smoker/on_process(delta_time)
	. = ..()
	var/mob/living/carbon/human/H = quirk_holder
