...
/datum/holocall
	...
	var/call_start_time
	///calls from a ADMIN PROBALLY autoconnect, if the receiving pad is not secure.
	var/head_call = FALSE
...
/datum/holocall/New(mob/living/call_source, obj/machinery/holopad/calling_pad, list/callees, elevated_access = FALSE)
	...
	user = call_source
	caller_location = get_area_name(call_source)
...
/datum/holocall/proc/Answer(obj/machinery/holopad/answering_holopad)
	...
	eye.setLoc(answering_holopad.loc, TRUE)
	hangup = new(eye, src)
	hangup.Grant(user)
