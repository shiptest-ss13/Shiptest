SUBSYSTEM_DEF(blackmarket)
	...
	var/shipping_method_descriptions = list(
		SHIPPING_METHOD_LAUNCH="Launches the item at the sector from across deep space, cheap but you might not recieve your item at all.",
		SHIPPING_METHOD_LTSRBT="Long-To-Short-Range-Bluespace-Transceiver, a machine that prepares items outside the sector and then teleports them to the location of the uplink."
	)
	...
	while(length(queued_purchases))
		...
		switch(purchase.method)
			...
			if(SHIPPING_METHOD_LAUNCH)
				...
				to_chat(recursive_loc_check(purchase.uplink.loc, /mob), "<span class='notice'>[purchase.uplink] flashes a message noting the order is being launched at the sector from [dir2text(startSide)].</span>")
