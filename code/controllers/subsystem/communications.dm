...
/datum/controller/subsystem/communications/proc/make_announcement(mob/living/user, is_silicon, input, auth_id)     //WS Edit - Make cap's announcement use logged-in name
	if(!can_announce(user, is_silicon))
		return FALSE
	if(is_silicon)
		minor_announce(html_decode(input),"[user.name] Announces:", from = user, zlevel = user.virtual_z())
		silicon_message_cooldown = world.time + COMMUNICATION_COOLDOWN_AI
	else
		priority_announce(html_decode(user.treat_message(input)), null, 'sound/misc/announce.ogg', "Captain", null, auth_id, user.virtual_z()) //WS Edit - Name on announcements & Make cap's announcement use logged-in name
