...
/obj/item/hilbertshotel/proc/promptAndCheckIn(mob/user)
    ...
    if(!storageTurf) //Blame subsystems for not allowing this to be in Initialize
        if(!GLOB.hhStorageTurf)
            ...
            var/datum/turf_reservation/storageReservation = SSmapping.request_dynamic_reservation(3, 3)
...
/obj/item/hilbertshotel/proc/tryStoredRoom(roomNumber, mob/user)
    if(storedRooms["[roomNumber]"])
        var/datum/turf_reservation/roomReservation = SSmapping.request_dynamic_reservation(hotelRoomTemp.width, hotelRoomTemp.height)
