...
/client/verb/verify_in_discord()
	...
	if(cached_one_time_token && cached_one_time_token != "")
		message = "You already generated your one time token, it is [cached_one_time_token], if you need a new one, you will have to wait until the round ends, or switch to another server, try verifying yourself in discord by using the command <span class=\"warning\">\"/verify one-time-token:[one_time_token]\"</span>"
		...
		message = "Your one time token is: [one_time_token], Assuming you have the required living minutes in game, you can now verify yourself in discord by using the command <span class=\"warning\">\"/verify one-time-token:[one_time_token]\"</span>"
