...
/obj/machinery/keycard_auth/proc/trigger_event(confirmer)
	...
	deadchat_broadcast(" triggered [event] at <span class='name'>[A1.name]</span>.", span_name("[triggerer]") , triggerer, message_type=DEADCHAT_ANNOUNCEMENT)

	var/area/A2 = get_area(confirmer)
	deadchat_broadcast(" confirmed [event] at <span class='name'>[A2.name]</span>.", span_name("[confirmer]") , confirmer, message_type=DEADCHAT_ANNOUNCEMENT)
	switch(event)
