/datum/unit_test/overmap_move/Run()
	var/datum/overmap/ship/S = new(list("x" = 1, "y" = 1), SSovermap.safe_sectors[1])
...
/datum/unit_test/overmap_dock/Run()
	var/datum/overmap/ship/controlled/docker = new(list("x" = 1, "y" = 1), SSovermap.safe_sectors[1], SSmapping.ship_purchase_list[pick(SSmapping.ship_purchase_list)]) //TODO: debug ship instead of picking random ship
	var/datum/overmap/dynamic/empty/dockee = new(list("x" = 1, "y" = 1), SSovermap.safe_sectors[1])
