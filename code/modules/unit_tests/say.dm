...
/datum/unit_test/translate_language/Run()
	...
	host_mob.grant_language(/datum/language/kalixcian_common, SPOKEN_LANGUAGE) // can speak but can't understand
	host_mob.add_blocked_language(subtypesof(/datum/language) - /datum/language/kalixcian_common, LANGUAGE_STONER)
	TEST_ASSERT_NOTEQUAL(surfer_quote, host_mob.translate_language(host_mob, /datum/language/kalixcian_common, surfer_quote), "Language test failed. Mob was supposed to understand: [surfer_quote]")
...
/*
	...
	listener.add_blocked_language(/datum/language/kalixcian_common)
	// speaking and whispering should be hearable
	conversation(distance = 1)
	// speaking should be hearable but not whispering
	conversation(distance = 5)
...
/datum/unit_test/speech/proc/conversation(distance = 0)
	speaker.forceMove(run_loc_bottom_left)
	listener.forceMove(locate((run_loc_bottom_left.x + distance), run_loc_bottom_left.y, run_loc_bottom_left.z))
...
/datum/unit_test/speech/proc/radio_test()
	speaker.forceMove(run_loc_bottom_left)
	listener.forceMove(locate((run_loc_bottom_left.x + 10), run_loc_bottom_left.y, run_loc_bottom_left.z))

	speaker_radio.forceMove(run_loc_bottom_left)
