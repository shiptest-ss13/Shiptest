...
/obj/item/reagent_containers/borghypo/attack(mob/living/carbon/M, mob/user)
	...
	if(R.total_volume && M.can_inject(user, user.zone_selected, inject_flags))
		...
		if(M.reagents)
			var/trans = R.trans_to(M, amount_per_transfer_from_this, transferred_by = user, methods = INJECT)
...
/obj/item/reagent_containers/borghypo/borgshaker/afterattack(obj/target, mob/user, proximity)
	...
	if(!proximity)
		...
		var/trans = R.trans_to(target, amount_per_transfer_from_this, transferred_by = user)
