/obj/item/inhaler
	...
	var/obj/item/reagent_containers/inhaler_canister/initial_canister_path
...
/obj/item/inhaler/Initialize(mapload)
	. = ..()
	if (ispath(initial_canister_path, /obj/item/reagent_containers/inhaler_canister))
		set_canister(new initial_canister_path)
...
/obj/item/inhaler/atom_destruction(damage_flag)
	. = ..()
...
/obj/item/inhaler/pre_attack(atom/interacting_with, mob/living/user, params)
...
/obj/item/inhaler/attackby(obj/item/tool, mob/living/user, params)
	if (istype(tool, /obj/item/reagent_containers/inhaler_canister))
		return try_insert_canister(tool, user, params)

	return ..()

/// Tries to remove the canister, if any is inserted.
/obj/item/inhaler/proc/try_remove_canister(mob/living/user, params)
	if (isnull(canister))
...
/obj/item/inhaler/proc/can_puff(mob/living/target_mob, mob/living/user, silent = FALSE)
	...
	if (HAS_TRAIT(carbon_target, TRAIT_NOBREATH))
		if (!silent)
			balloon_alert(user, "not breathing!")
		return FALSE
	var/obj/item/organ/lungs/lungs = carbon_target.getorganslot(ORGAN_SLOT_LUNGS)
...
/obj/item/reagent_containers/inhaler_canister
	...
	possible_transfer_amounts = list(5) // hack
	...
	var/pre_insert_sound = 'sound/items/tape_flip.ogg'
	/// The sound that plays when we are removed or inserted.
	var/post_insert_sound = 'sound/items/taperecorder_close.ogg'
...
/obj/item/reagent_containers/inhaler_canister/atom_destruction(damage_flag)
	...
	var/datum/effect_system/smoke_spread/chem/thin/smoke = new()
	smoke_reagents.my_atom = src
	for (var/datum/reagent/reagent as anything in reagents.reagent_list)
		smoke_reagents.add_reagent(reagent.type, reagent.volume)
		reagents.remove_reagent(reagent.type, reagent.volume)
	if (smoke_reagents.reagent_list)
		smoke.set_up(smoke_reagents, 1, get_turf(src))
		smoke.start()
		visible_message(span_warning("[src] breaks open and sprays its aerosolized contents everywhere!"))
	else
...
/obj/item/inhaler/sting
	name = "sting inhaler"
	icon_state = "inhaler_generic"
	initial_canister_path = /obj/item/reagent_containers/inhaler_canister/sting
...
/obj/item/inhaler/cocktail
	...
	initial_canister_path = /obj/item/reagent_containers/inhaler_canister/combat_drug
...
/obj/item/inhaler/placebatol
	...
	icon_state = "inhaler_medical"
	initial_canister_path = /obj/item/reagent_containers/inhaler_canister/placebatol
