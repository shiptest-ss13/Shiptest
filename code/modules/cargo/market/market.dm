...
/datum/cargo_market/black
	name = "Black Market"
	supply_packs = /datum/supply_pack/blackmarket
...
/datum/cargo_market/black/cycle_stock()
	...
	for(var/category in supply_packs)
		for(var/item in supply_packs[category])
...
/datum/cargo_market/black/proc/get_item_in_market(datum/supply_pack/blackmarket/item)
	for(var/item_to_find in supply_packs[item.category])
...
/datum/cargo_market/black/proc/add_item(datum/supply_pack/blackmarket/item, paired)
	...
	if(!(item.category in categories))
		...
		supply_packs[item.category] = list()

	supply_packs[item.category] += item
...
/datum/cargo_market/black/proc/purchase(item, category, method, obj/item/blackmarket_uplink/uplink, user)
	...
	for(var/datum/supply_pack/blackmarket/I in supply_packs[category])
		if(I.type != item)
			continue
