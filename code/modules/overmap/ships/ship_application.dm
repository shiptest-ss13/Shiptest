/datum/ship_application
	/// The crew this application is linked to.
	var/datum/crew/crew
...
/datum/ship_application/New(mob/dead/new_player/applicant, datum/crew/_crew)
	...
	RegisterSignal(crew, COMSIG_PARENT_QDELETING, .proc/important_deleting_during_apply)

/datum/ship_application/Destroy()
	SStgui.close_uis(src)
	if(status != SHIP_APPLICATION_UNFINISHED && status != SHIP_APPLICATION_CANCELLED)
		LAZYREMOVE(crew.applications, ckey(app_key))
		if(app_mob)
			SEND_SOUND(app_mob, sound('sound/misc/server-ready.ogg', volume=50))
			to_chat(app_mob, "<span class='warning'>Your application to [crew.name] has been deleted.</span>", MESSAGE_TYPE_INFO)
	app_mob = null
	crew = null
	. = ..()
...
/datum/ship_application/proc/get_user_response()
	...
	UnregisterSignal(crew, COMSIG_PARENT_QDELETING)
	LAZYSET(crew.applications, ckey(app_key), src)

	if(crew.owner_mob != null)
		// don't need to use check_blinking, because it DAMN well better be blinking now that we exist
		crew.owner_act.set_blinking(TRUE)
		SEND_SOUND(crew.owner_mob, sound('sound/misc/server-ready.ogg', volume=50))
		var/message = \
		...
		to_chat(crew.owner_mob, message, MESSAGE_TYPE_INFO)
	return TRUE
...
/datum/ship_application/proc/application_status_change(new_status)
	...
	switch(status)
		if(SHIP_APPLICATION_ACCEPTED)
			to_chat(app_mob, "<span class='notice'>Your application to [crew.name] was accepted!</span>", MESSAGE_TYPE_INFO)
		if(SHIP_APPLICATION_DENIED)
			to_chat(app_mob, "<span class='warning'>Your application to [crew.name] was denied!</span>", MESSAGE_TYPE_INFO)
