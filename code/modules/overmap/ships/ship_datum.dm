 ...
 * * delta_time - The time since the last burn tick
 */
/datum/overmap/ship/proc/burn_engines(percentage = 100, delta_time)
	if(docked_to || docking)
		CRASH("Ship burned engines while docking or docked!")

	return acceleration_speed * (percentage / 100) * delta_time
