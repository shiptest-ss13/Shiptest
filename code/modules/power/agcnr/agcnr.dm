// Defines for AGCNR (AKA the RBMK) stuff
...
Constricted Plasma: Like plasma, but more efficient at its job.
...
/obj/machinery/atmospherics/components/trinary/nuclear_reactor
	...
	var/vessel_max_repair_integrity = 350 //The maximum integrity you can reach using flex-seal to repair the reactor
...
/obj/machinery/atmospherics/components/trinary/nuclear_reactor/welder_act(mob/living/user, obj/item/I)
	...
	if(I.use_tool(src, user, 0, volume=40))
		if(vessel_integrity > 0.5 * initial(vessel_integrity))
			to_chat(user, "<span class='notice'>[src] is free from cracks. Further repairs must be carried out with sealant.</span>")
			return FALSE
		vessel_integrity += 20
...
/obj/machinery/atmospherics/components/trinary/nuclear_reactor/proc/handle_alerts()
	...
	if(warning)
		...
		relay('sound/effects/agcnr/alarm.ogg', null, loop=TRUE, channel = CHANNEL_REACTOR_ALERT)
		set_light(0)
...
/obj/machinery/atmospherics/components/trinary/nuclear_reactor/proc/relay(var/sound, var/message=null, loop = FALSE, channel = null) //Sends a sound + text message to the crew of a ship
	for(var/mob/M in GLOB.player_list)
		if(M.get_virtual_level() == get_virtual_level())
...
/obj/machinery/atmospherics/components/trinary/nuclear_reactor/proc/start_up()
	...
	AR.ambientsounds = 'sound/effects/agcnr/reactor_hum.ogg'
	var/startup_sound = pick('sound/effects/agcnr/startup.ogg', 'sound/effects/agcnr/startup2.ogg')
...
/obj/machinery/atmospherics/components/trinary/nuclear_reactor/proc/shut_down()
	...
	set_light(0)
	var/area/AR = get_area(src)
	AR.ambientsounds = initial(AR.ambientsounds)
	K = 0
	desired_k = 0
	temperature = 0
	update_icon()
...
/obj/machinery/computer/reactor/fuel_rods/attack_hand(mob/living/user)
	...
	if(!fuel_rod)
		return
	playsound(src, "switch", 100, FALSE)
	playsound(reactor, 'sound/effects/agcnr/freespace2/crane_1.wav', 100, FALSE)
...
/obj/machinery/computer/reactor/pump/attack_hand(mob/living/user)
	...
	if(!is_operational())
		return FALSE
	playsound(loc, "switch", 100, FALSE)
...
/datum/computer_file/program/nuclear_monitor/run_program(mob/living/user)
	...
	if(!reactor)
		for(var/obj/machinery/atmospherics/components/trinary/nuclear_reactor/R in GLOB.machines)
			if(R.get_virtual_level() == usr.get_virtual_level())
				reactor = R
				break
	active = TRUE
