/datum/surgery_step/omni/prepare_bone
	name = "Prepare Bone"
	implements = list(
		TOOL_BONEGEL = 100)
...
/datum/surgery_step/omni/prepare_bone/test_op(mob/user, mob/living/target,datum/surgery/omni/surgery)
	if(istype(surgery.last_step,/datum/surgery_step/omni/prepare_bone))
		return FALSE
	return TRUE
...
/datum/surgery_step/omni/add_prosthetic/test_op(mob/user, mob/living/target,datum/surgery/omni/surgery)
	if(!istype(surgery.last_step,/datum/surgery_step/omni/prepare_bone))
		return FALSE
	return TRUE
...
/datum/surgery_step/omni/add_prosthetic/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	if(istype(tool, /obj/item/organ_storage))
