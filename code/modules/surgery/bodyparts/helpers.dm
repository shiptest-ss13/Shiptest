...
// FUCK YOU AUGMENT CODE - With love, MeeM
/mob/living/carbon/proc/new_body_part(zone, variant, fixed_icon, datum/species/species)
	species ||= dna.species
	var/obj/item/bodypart/L
	switch(zone)
		if(BODY_ZONE_L_ARM)
			switch(variant)
				if(PREF_VARIANT_NONE)
					L = new species.species_l_arm()
				if(PREF_VARIANT_FBP)
					L = new species.species_robotic_l_arm()
				if(PREF_VARIANT_PHORID)
					L = new species.species_phorid_l_arm()
		if(BODY_ZONE_R_ARM)
			switch(variant)
				if(PREF_VARIANT_NONE)
					L = new species.species_r_arm()
				if(PREF_VARIANT_FBP)
					L = new species.species_robotic_r_arm()
				if(PREF_VARIANT_PHORID)
					L = new species.species_phorid_r_arm()
		if(BODY_ZONE_HEAD)
			switch(variant)
				if(PREF_VARIANT_NONE)
					L = new species.species_head()
				if(PREF_VARIANT_FBP)
					L = new species.species_robotic_head()
				if(PREF_VARIANT_PHORID)
					L = new species.species_phorid_head()
		if(BODY_ZONE_L_LEG)
			if(species.is_digitigrade(src))
				switch(variant)
					if(PREF_VARIANT_NONE)
						L = new species.species_digi_l_leg()
					if(PREF_VARIANT_FBP)
						L = new species.species_robotic_digi_l_leg()
					if(PREF_VARIANT_PHORID)
						L = new species.species_phorid_digi_l_leg()
			else
				switch(variant)
					if(PREF_VARIANT_NONE)
						L = new species.species_l_leg()
					if(PREF_VARIANT_FBP)
						L = new species.species_robotic_l_leg()
					if(PREF_VARIANT_PHORID)
						L = new species.species_phorid_l_leg()
		if(BODY_ZONE_R_LEG)
			if(species.is_digitigrade(src))
				switch(variant)
					if(PREF_VARIANT_NONE)
						L = new species.species_digi_r_leg()
					if(PREF_VARIANT_FBP)
						L = new species.species_robotic_digi_r_leg()
					if(PREF_VARIANT_PHORID)
						L = new species.species_phorid_digi_r_leg()
			else
				switch(variant)
					if(PREF_VARIANT_NONE)
						L = new species.species_r_leg()
					if(PREF_VARIANT_FBP)
						L = new species.species_robotic_r_leg()
					if(PREF_VARIANT_PHORID)
						L = new species.species_phorid_r_leg()
		if(BODY_ZONE_CHEST)
			switch(variant)
				if(PREF_VARIANT_NONE)
					L = new species.species_chest()
				if(PREF_VARIANT_FBP)
					L = new species.species_robotic_chest()
				if(PREF_VARIANT_PHORID)
					L = new species.species_phorid_chest()
	. = L

/mob/living/carbon/monkey/new_body_part(zone, variant, fixed_icon, datum/species/species)
	var/obj/item/bodypart/L
	switch(zone)
		if(BODY_ZONE_L_ARM)
	...
	if(L)
		...
		if(variant == PREF_VARIANT_FBP)
			L.change_bodypart_status(BODYTYPE_ROBOTIC)
	. = L

/mob/living/carbon/alien/larva/new_body_part(zone, variant, fixed_icon, datum/species/species)
	var/obj/item/bodypart/L
	switch(zone)
		if(BODY_ZONE_HEAD)
...
/mob/living/carbon/alien/humanoid/new_body_part(zone, variant, fixed_icon, datum/species/species)
	var/obj/item/bodypart/L
	switch(zone)
		if(BODY_ZONE_L_ARM)
