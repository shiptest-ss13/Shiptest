...
/obj/item/organ/liver/on_life(seconds_per_tick, times_fired)
	...
	..()	//perform general on_life(seconds_per_tick, times_fired)
	if(istype(C))
		if(!(organ_flags & ORGAN_FAILING) && !HAS_TRAIT(C, TRAIT_NOMETABOLISM))//can't process reagents with a failing liver
			...
			if(filterToxins && !HAS_TRAIT(owner, TRAIT_TOXINLOVER))
				...
				for(var/datum/reagent/toxin/T in C.reagents.reagent_list)
					...
					if (thisamount && thisamount <= toxTolerance)
						C.reagents.remove_reagent(T.type, 0.5 * seconds_per_tick)
					else
			...
			//metabolize reagents
			C.reagents.metabolize(C, seconds_per_tick, times_fired, can_overdose=TRUE)
			...
			C.liver_failure(seconds_per_tick, times_fired)
