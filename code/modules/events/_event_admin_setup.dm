...
/datum/event_admin_setup/text_input/prompt_admins()
	...
	chosen = input(usr, input_text, event_control.name, suggestion) as null|text
	if(!chosen)
		return ADMIN_CANCEL_EVENT
...
/datum/event_admin_setup/input_number/prompt_admins()
	chosen_value = input(usr, input_text, event_control.name) as null|num
...
/datum/event_admin_setup/multiple_choice/prompt_admins()
	...
	//AGHHHH need to port this but way out of scope. these will be broken
