...
/obj/machinery/photocopier/attackby(obj/item/O, mob/user, params)
	if(istype(O, /obj/item/paper))
		...
		if(isinspace())
			to_chat(user, span_warning("There's nothing to fasten [src] to!"))
			return
		playsound(loc, O.usesound, 50, 1)
		to_chat(user, span_warning("You start [anchored ? "unwrenching" : "wrenching"] [src]..."))
		if(do_after(user, 20*O.toolspeed, target = src))
			if(QDELETED(src))
				return
			to_chat(user, span_notice("You [anchored ? "unwrench" : "wrench"] [src]."))
			anchored = !anchored
	else if(istype(O, /obj/item/areaeditor/blueprints))
		to_chat(user, span_warning("The Blueprint is too large to put into [src]. You need to find something else to record the document."))
	else
		return ..()
