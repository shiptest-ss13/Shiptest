...
#warn these will all have to be redefined, and likely reset? remember to raise savefile numbers, and clear out this file of commented lines
...
/datum/preferences/proc/load_character(slot)
	...
	#warn cleanse all this shit -- every commented line.

	#warn Temp nuprefs character load goes here.
	var/oldcd = S.cd // bad var i'm doing to be ridiculously conservative and not duplicate [slot] unnecessarily
	S.cd = "/character_nuprefs[slot]"
	...
	for(var/datum/preference/pref as anything in GLOB.pref_topo_order)
		var/list/dep_values = assemble_pref_dep_list(pref, pref_values)
		if(!pref.is_available(dep_values))
			pref_values[pref.type] = PREFERENCE_ENTRY_UNAVAILABLE
			// if the preference is unavailable, no reason to continue with deserialization, as deserializing any value from the file would overwrite its unavailability.
			continue

		// we fill in the default value before even trying to deserialize. this protects against runtimes and invalid values
		...
		if(change_invalid)
			// if the deserialized data was invalid, we need to keep track of the error and avoid writing anything else.
			// we already wrote the default value before, and that value is never invalid, so we don't need to worry about violating correctness
			errors += change_invalid
		else
			pref_values[pref.type] = pref_deserial_data

	S.cd = oldcd // gotta reset it, because i'm paranoid
...
/datum/preferences/proc/save_character()
	...
	#warn Temp nuprefs character save goes here.
	...
	#warn could maybe use an | for this. in the future, this should be "abortable" if there are file-writing errors (not that i'm sure what would cause those)
	...
	// this list matches pref DATUMS to their serialized values, because it makes things a bit easier for us.
	// technically, we could save immediately after serializing each value, instead of in batches, but this makes errors less dangerous
	var/list/serialized_pref_data = list()
	for(var/p_t in pref_values)
		var/datum/preference/p_datum = GLOB.pref_type_lookup[p_t]
		serialized_pref_data[p_datum] = p_datum.serialize(pref_values[p_t])

	var/oldcd = S.cd // bad var i'm doing to be ridiculously conservative and not duplicate [default_slot] unnecessarily
	S.cd = "/character_nuprefs[default_slot]"
