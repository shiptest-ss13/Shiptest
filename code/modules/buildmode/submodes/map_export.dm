...
/datum/buildmode_mode/export/change_settings(client/target_client)
	...
	to_chat(target_client, span_notice("[what_to_change] is now [save_flag & options[what_to_change] ? "ENABLED" : "DISABLED"]."))

/datum/buildmode_mode/export/show_help(client/target_client)
	to_chat(target_client, span_notice("***********************************************************"))
	to_chat(target_client, span_notice("Left Mouse Button on turf/obj/mob      = Select corner"))
	to_chat(target_client, span_notice("Right Mouse Button on buildmode button = Set export options"))
	to_chat(target_client, span_notice("***********************************************************"))
