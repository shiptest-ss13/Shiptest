...
/mob/living/simple_animal/slime/handle_status_effects(seconds_per_tick, times_fired)
	..()
	if(!stat && SPT_PROB(16, seconds_per_tick))
...
/mob/living/simple_animal/slime/proc/handle_feeding(seconds_per_tick, times_fired)
	...
	if(iscarbon(M))
		...
		C.adjustCloneLoss(rand(2, 4) * 0.5 * seconds_per_tick)
		C.adjustToxLoss(rand(1, 2) * 0.5 * seconds_per_tick)

		if(SPT_PROB(5, seconds_per_tick) && C.client && !HAS_TRAIT(C, TRAIT_ANALGESIA))
		...
		totaldamage += SA.adjustCloneLoss(rand(2, 4) * 0.5 * seconds_per_tick)
		totaldamage += SA.adjustToxLoss(rand(1, 2) * 0.5 * seconds_per_tick)
