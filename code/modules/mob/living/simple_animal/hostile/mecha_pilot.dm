...
/mob/living/simple_animal/hostile/human/ramzi/mecha_pilot/makossowarra //makossowarra are syndies! no it's just a weird path.
	...
	faction = list("makossowarra")
	spawn_mecha_type = /obj/mecha/combat/marauder/loaded

/mob/living/simple_animal/hostile/human/ramzi/mecha_pilot/no_mech/makossowarra
	name = "\improper Makosso-Warra Mecha Pilot"
	desc = "Death to the Syndicate. This variant comes in MECHA DEATH flavour."
	icon_living = "makossowarra"
	icon_state = "makossowarra"
...
/mob/living/simple_animal/hostile/human/ramzi/mecha_pilot/proc/exit_mecha(obj/mecha/M)
	if(!M)
		return 0
...
/mob/living/simple_animal/hostile/human/ramzi/mecha_pilot/proc/is_valid_mecha(obj/mecha/M)
	if(!M)
		return 0
	if(M.occupant)
		return 0
...
/mob/living/simple_animal/hostile/human/ramzi/mecha_pilot/proc/mecha_face_target(atom/A)
	if(mecha)
...
/mob/living/simple_animal/hostile/human/ramzi/mecha_pilot/proc/mecha_reload()
	if(mecha)
		for(var/equip in mecha.equipment)
			var/obj/item/mecha_parts/mecha_equipment/ME = equip
			if(ME.needs_rearm())
...
/mob/living/simple_animal/hostile/human/ramzi/mecha_pilot/proc/get_mecha_equip_by_flag(flag = MECHA_RANGED)
	. = list()
	if(mecha)
		for(var/equip in mecha.equipment)
			var/obj/item/mecha_parts/mecha_equipment/ME = equip
...
//Fire it
/mob/living/simple_animal/hostile/human/ramzi/mecha_pilot/OpenFire(atom/A)
	if(mecha)
		mecha_reload()
		mecha_face_target(A)
...
/mob/living/simple_animal/hostile/human/ramzi/mecha_pilot/AttackingTarget()
	if(mecha)
...
/mob/living/simple_animal/hostile/human/ramzi/mecha_pilot/handle_automated_action()
	if(..())
		if(!mecha)
...
/mob/living/simple_animal/hostile/human/ramzi/mecha_pilot/death(gibbed)
	if(mecha)
		mecha.aimob_exit_mech(src)
	..()

/mob/living/simple_animal/hostile/human/ramzi/mecha_pilot/gib()
	if(mecha)
		mecha.aimob_exit_mech(src)
	..()
...
//~simple animals~
/mob/living/simple_animal/hostile/human/ramzi/mecha_pilot/CanAttack(atom/the_target)
...
/mob/living/simple_animal/hostile/human/ramzi/mecha_pilot/EscapeConfinement()
	if(mecha && loc == mecha)
		return 0
	..()


/mob/living/simple_animal/hostile/human/ramzi/mecha_pilot/Move(NewLoc,Dir=0,step_x=0,step_y=0)
	if(mecha && loc == mecha)
		return mecha.relaymove(src, Dir)
	return ..()


/mob/living/simple_animal/hostile/human/ramzi/mecha_pilot/Goto(target, delay, minimum_distance)
	if(mecha)
