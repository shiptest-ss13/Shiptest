GLOBAL_DATUM_INIT(typing_indicator_icon, /mutable_appearance, mutable_appearance('icons/mob/talk.dmi', "default0", -HALO_LAYER))
...
/mob/living/create_typing_indicator()
	if(!typing_indicator && stat == CONSCIOUS) //Prevents sticky overlays and typing while in any state besides conscious
		add_overlay(GLOB.typing_indicator_icon)
		typing_indicator = TRUE

/mob/living/remove_typing_indicator()
	if(typing_indicator)
		cut_overlay(GLOB.typing_indicator_icon)
